...
abstract: This Data Management Plan describes the cleaning data sets that were undertaken with three administrative datasets. Three challenges were faced during this project. The first one was related to efficiency. Due to the size in the storage of the datasets, some variables and individuals were prioritized for further analysis. The second one was the identification of selectivity issues that arouse in administrative data. One selectivity issue was the case of attrition or dropouts, also it was the case with defiers or pupils who were eligible for a conditional cash transfer but did not participate. Finally, the third issue is the challenge imposed by Big Data in applied economics and the necessity to explore software like R.
output: 
  pdf_document:
...
#if (!require("rapportools")) {install.packages("rapportools"); library(rapportools)}
...
```{r cleanscore, eval=FALSE}
  ...
  head(score_c)
  Cols.Chosen<-c("num_assets", "puntaje","documen", "ingresos" )
...
admin_educ_practice <- admin_practice[educ_practice, on = .(DOCUMENTO = NRO_DOCUMENTO_2014)]
nomatch=0
...
admin_project <- rename(admin_practice, c("DOCUMENTO"="ID"))
educ_project <- rename(educ_practice, c("NRO_DOCUMENTO_2014"="ID"))
score_project <- rename(score_practice, c("documen"="ID"))
...
#Inner join
score_educ<-merge(score, educ, by.x = "ID", by.y = "ID") %>% print
  dim(score_educ)

#Left join
score_educ_admin <- merge(score_educ, admin, by = "ID", all.x = TRUE) %>% print
  dim(score_educ_admin)
  
```

```{r verifc, eval=TRUE}

treated <- ifelse(!is.na(score_educ_admin$CODBENEFICIARIO_MFA),1,0)
  table(treated) 

dummy_puntaje <- ifelse(!is.na(score_educ_admin$puntaje),1,0)
...
setDF(score_educ_admin)

class(score_educ_admin)

# Save an object to a file
  saveRDS(score_educ_admin, file = "./data/processed/score_educ_admin.rds")

write.csv(score_educ_admin,'./data/processed/score_educ_admin.csv')

# Restore the object
  score_educ_admin <- readRDS(file = "./data/processed/score_educ_admin.rds")
...
score_educ_admin$treated <- ifelse(!is.na(score_educ_admin$CODBENEFICIARIO_MFA),1,0)
  table(treated) 
  
treated.f <- factor(treated, levels= c(0,1),
  labels = c("Comparison", "Beneficiarie"))

score_educ_admin$treated_f <- factor(treated, levels= c(0,1),
  labels = c("Comparison", "Beneficiarie"))

str(score_educ_admin)
...
```{r aboutrd, eval=TRUE}
# Kernel Density Plot
  d1 <- density(score_educ_admin$puntaje) # returns the density data
  plot(d1, main = "Kernel density of score") # plots the results

# Density plots using a more stylish code  
  p <- ggplot(score_educ_admin, aes(x=puntaje)) + geom_density()
  p
  p+ geom_vline(aes(xintercept=(weight=30.1)) ,
            color="blue", linetype="dashed", size=1)

  newgraph <- ggplot(score_educ_admin, aes(x=puntaje, color=treated.f)) +
  geom_density()
  newgraph

# Add mean lines

mean_sco <- ddply(score_educ_admin, "treated.f", summarise, grp.mean=mean(puntaje))

compagraph <-ggplot(score_educ_admin, aes(x=puntaje, color=treated.f)) +
  geom_density()+
...
ggplot(score_educ_admin, aes(treated, puntaje)) +
        geom_boxplot()
...
ggplot(score_educ_admin, aes(treated, num_retakes)) +
        geom_boxplot()
```

```{r distr3, eval= TRUE}
ggplot(score_educ_admin, aes(treated, num_assets)) +
        geom_boxplot()
...
As mentioned in the purpose of thisData Managment Project, one of the purpose is also to show the potential use of these data as inputs for an impact evaluation and therfore for decision policy making. To do that a general overview of a regression discontinuity design. However, deeper analysis is not presented here.

To show this potential use the RDROBUST command is used.

[sites.google.com/site/rdpackages](https://sites.google.com/site/rdpackages/home)
...
- rdrobust: inference and graphical procedures using local polynomial and partitioning regression methods.
- rdlocrand: finite-sample inference using local randomization methods.
...
- rdpower: power and sample size calculations using robust bias-corrected local polynomial inference.
...
During the develpment of this data managment project some issues occurred in regard with R and RMarkdown implementation. Programming in R has been a challenging task since it is a constant developing software. 


When importing the data some issues were faced. Some related to the libraries that requiere other libraries. For example:

>> Error: When i is a data.table (or character vector), the columns to join by must be specified using 'on=' argument (see ?data.table), by keying x (i.e. sorted, and, marked as sorted, see ?setkey), or by sharing column names between x and i (i.e., a natural join). Keyed joins might have further speed benefits on very large data due to x being sorted in RAM.

When reading the first time, some columns are type 'integer64' but package bit64 is not installed. Those columns will print as strange looking floating point data. There is no need to reload the data. Simply install.packages('bit64') to obtain the integer64 print method and print the data again.

![import.](D:\2_2019_Colombia\data_fa\while_reading_score.png)


It was neccesary to debug R and Latex. Useful information was found

[Source of R Markdown](https://yihui.org/tinytex/r/#debugging) 


![rmark.](D:\2_2019_Colombia\data_fa\rmarkdown.png)

One importat issue is that the so called _*BIG DATA*_ comes not only with opportunities but also with challenges. Whether it comes from administrative data, as in the present case, or it comes from other kind of sources of information. The following error shows that a simple statistical summary can fail when the data anlayez is big enough to not be supported by one computer.

![rderr.](D:\2_2019_Colombia\data_fa\while_rd.png)

# Conclusions

* R is an effective data handling and storage software, and it offers several different ways to perform the same function. Thus it is challenging to manage.
* The dataset were _reduce_ in size to properly perform the data cleaning and to create one _clean_ easily usable data set. 
* The final dataset *score_educ_admin.csv* can be open in other softwares such as STATA or SPSS.
* During the data cleaning some important selectivity issues were found and discussed. One being attrition. In terms of educational indicators, attittion represents dropouts, pupils who leave school. A second selectivity issue unexceptedly found was re-enrrollments, which corresponds to pupils how comeback to school.
* Due to the size of the files only individuals without droupouts or reenrrollments were kept. However, this is an important issue to consider for future research.
* A third selectivity issue identified was related to individuals who were not beneficieries of the program, or comparison group. Some of them were eligible for the program but are not part of it. This group of pupils or _defiers_ are of interest in further research about the impacts of the program.
* Administrative data is a powerful source of information to perform research and perform impact evaluation analysis. Nonetheless, it is demanding in terms of data cleaning. 
* In this data managment project is shown taht by combinging three different types of administrative data from one country a _Regression Discontinuity_ estimation can be perform.
* Big data is a challenging really for applied economics
